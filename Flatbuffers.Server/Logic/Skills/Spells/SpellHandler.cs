...
public class SpellHandler : ISpellHandler
	{
		...
		public virtual void CasterMoves()
		{
			...
			if (Caster is GamePlayer)
				(Caster as GamePlayer).Out.SendMessage(LanguageMgr.GetTranslation((Caster as GamePlayer).Network, "SpellHandler.CasterMove"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
		}
		...
		protected virtual void FocusSpellAction(GameEvent e, object sender, EventArgs args)
		{
			...
			GameEventManager.RemoveHandler(Caster, GameLivingEvent.Dying, new GameEventHandler(FocusSpellAction));
			GameEventManager.RemoveHandler(Caster, GameLivingEvent.AttackedByEnemy, new GameEventHandler(FocusSpellAction));
			GameEventManager.RemoveHandler(currentEffect.Owner, GameLivingEvent.Dying, new GameEventHandler(FocusSpellAction));
