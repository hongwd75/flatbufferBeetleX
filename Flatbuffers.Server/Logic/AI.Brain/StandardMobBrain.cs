...
public class StandardMobBrain : APlayerVicinityBrain, IOldAggressiveBrain
	{
		...
		protected virtual void BringFriends(GameLiving attacker)
		{
			...
			if (RandomUtil.Chance(percentBAF % 100))
				maxAdds++;
					...
					foreach (GameNPC npc in Body.GetNPCsInRadius(range))
					{
						...
						if (npc.IsFriend(Body) && npc.IsAggressive && npc.IsAvailable && npc.Brain is StandardMobBrain brain)
						{
							...
							if (victims != null && victims.Count > 0)
								target = victims[RandomUtil.Int(0, victims.Count - 1)];
							else
								target = puller;
		...
		public virtual bool CheckSpells(eCheckSpellType type)
		{
			...
			if (Body != null && Body.Spells != null && Body.Spells.Count > 0)
			{
				...
				if (type == eCheckSpellType.Defensive)
				{
					...
					{
						spellToCast = (Spell)spell_rec[RandomUtil.Int((spell_rec.Count - 1))];
                ...
                case "DAMAGESHIELD":
                    {
						...
						if (!LivingHasEffect(Body, spell) && !Body.AttackState && RandomUtil.Chance(40) && spell.Target.ToLower() != "pet")
						{
							Body.TargetObject = Body;
							break;
						}
						if (Body.ControlledBrain != null && Body.ControlledBrain.Body != null && RandomUtil.Chance(40) && Body.GetDistanceTo(Body.ControlledBrain.Body) <= spell.Range && !LivingHasEffect(Body.ControlledBrain.Body, spell) && spell.Target.ToLower() != "self")
						{
		...
		public virtual Coordinate GetRandomWalkTarget()
        {
            ...
            double targetX = Body.SpawnPosition.X + RandomUtil.Int(-maxRoamingRadius, maxRoamingRadius);
            double targetY = Body.SpawnPosition.Y + RandomUtil.Int(-maxRoamingRadius, maxRoamingRadius);
